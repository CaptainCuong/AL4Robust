...
# print(min(new_file['ASR_DeepWordBug']))
# print(max(new_file['ASR_DeepWordBug']))
# print(mean(new_file['ASR_DeepWordBug']))
# raise
...
for t in itertools.count():
    ...
    if (max(r2_rf) > args.r2_threshold_inter and t > args.min_iters_inter):
        ...
        ax.invert_yaxis()
        title = ('BERT ' if args.sample_data_file == 'data_bert.csv' else 'RoBERTa ') + 'Interpolation'
        ax.set_title(title)
...
for i, ft in enumerate(discrete_fts+continuous_fts):
    ...
    ax.set_title(f"{'BERT' if model == '_bert' else 'RoBERTa'} Interpolation")
